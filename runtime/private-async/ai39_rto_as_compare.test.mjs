...
test("trend profile selector is allowlisted and cannot accept arbitrary ASNs", () => {
  ...
  assert.deepEqual(validateRtoComparePayload({ profile: "allied_external_control" }), { profile: "allied_external_control" });
...
test("rto_core compares exactly three fixed ASNs with one Shodan Trends request each", async () => {
  ...
  const fetchImpl = async (url) => {
    ...
    const asn = new URL(text).searchParams.get("query").replace("asn:", "");
...
test("neighbour_control uses only Miranda and two Ugletelecom ASNs", async () => {
  ...
  const result = await runAi39RtoAsCompare({ profile: "neighbour_control" }, { fetchImpl, shodanKey: "secret", paceMs: 0 });
...
test("allied_external_control is fixed to three unrelated allied-country large networks", async () => {
  const fetchImpl = async (url) => {
    const text = String(url);
    if (text.includes("stat.ripe.net")) return response(200, { data: { prefixes: [{}] } });
    return response(200, trends([["2026-01", 1000], ["2026-08", 900]]));
  };
  const result = await runAi39RtoAsCompare({ profile: "allied_external_control" }, { fetchImpl, shodanKey: "secret", paceMs: 0 });
  assert.deepEqual(result.asns, ["AS3320", "AS2856", "AS7018"]);
  assert.equal(result.rows.every((row) => row.shodan_trends.summary.first_to_latest_pct === -10), true);
});
