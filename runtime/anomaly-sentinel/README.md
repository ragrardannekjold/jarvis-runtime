...
- Workflow records that GitHub still reports `active` after their source disappeared from the pinned default-branch tree are emitted as historical rerun-surface evidence and force `RED`; they no longer abort the rest of the observation.
...
The Sentinel itself still runs on GitHub's scheduler, so it cannot detect a total GitHub scheduler outage with an independent clock. GitHub also does not document a server-side ordering or updated-time cursor that can completely discover an arbitrary rerun from the 30-day rerun window, and the Actions inventory can retain `active` records whose source is absent from the current tree. Those limits, plus independently enforced branch/ruleset protection, require an independently clocked, signature-validating durable observer/webhook and explicit historical rerun neutralization before any write authority, incident closure, or certified `GREEN` may be enabled.
