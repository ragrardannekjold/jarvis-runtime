...
UA='KyivV3PublicCollector/1.1 (+defensive-civilian-safety; passive-only)'
REGIONS={
 'BRY':('Bryansk','https://radar.cloudflare.com/traffic/571473'),
 'KUR':('Kursk','https://radar.cloudflare.com/traffic/538555'),
 'BEL':('Belgorod','https://radar.cloudflare.com/traffic/578071'),
 'VOR':('Voronezh','https://radar.cloudflare.com/traffic/472039'),
}
...
def fetch(url,cls,semantic,receipts,timeout=25):
    ...
    try:
        req=urllib.request.Request(url,headers={'User-Agent':UA,'Accept':'application/json,text/html,*/*;q=0.5'})
...
def resolve(name,receipts):
    ...
    o=jfetch(f'{IODA}/entities/query?{q}','relationship_metadata',f'IODA aggregate region resolution for {name}',receipts)
...
def main():
    ...
    covered=0; pivots=0
    for tag,(name,radar) in REGIONS.items():
        rv=p['cybint']['regions'][tag]
        routing_ok=rv.get('ripe_ris',{}).get('status')=='OK' and rv.get('ripe_ris',{}).get('fresh') is True
        if not routing_ok:
            fb=ioda_bgp(tag,name,receipts,anchor);rv['routing_fallback']=fb;routing_ok=fb.get('status')=='OK' and fb.get('fresh') is True
        else:rv['routing_fallback']={'status':'NOT_NEEDED'}
        active_ok=rv.get('ioda_active',{}).get('status')=='OK' and rv.get('ioda_active',{}).get('fresh') is True
        rv['technical_two_class_coverage']=bool(active_ok and routing_ok)
        covered+=int(rv['technical_two_class_coverage'])
        st,raw=fetch(radar,'regional_documentary_relationship',f'Cloudflare Radar public regional relationship pivot {tag}',receipts)
        rv['documentary_relationship_pivot']={'source':'CLOUDFLARE_RADAR_PUBLIC_REGION_PAGE','status':'CHECKED' if st==200 and raw else 'UNKNOWN','http_status':st}
        pivots+=int(st==200 and bool(raw))
    p['cybint']['regions_two_class_covered']=covered
    p['cybint']['documentary_relationship_regions_checked']=pivots
    p['cybint']['regional_coverage_gate']='PASS' if covered==4 and pivots>=3 else 'INCOMPLETE'
    p['cybint']['independence_note']='IODA active probing and BGP are distinct measurement classes; Bryansk may use same-provider IODA BGP fallback, while RIPE RIS supplies independent routing lineage elsewhere. Cloudflare regional pages are documentary pivots only, not a technical class.'
    R.write_text(''.join(json.dumps(x,ensure_ascii=False)+'\n' for x in receipts),encoding='utf-8')
    p['receipt_count']=len(receipts);p['receipt_digest']=compact(receipts)
    P.write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(json.dumps({'regions_two_class_covered':covered,'documentary_relationship_regions_checked':pivots,'regional_coverage_gate':p['cybint']['regional_coverage_gate'],'receipt_count':len(receipts)},ensure_ascii=False))
