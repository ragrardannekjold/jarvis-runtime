...
UA = "KyivV3PublicCollector/1.0 (+defensive-civilian-safety; passive-only)"
...
def fetch(url, receipts, cls, semantic, *, payload=None, timeout=30):
    ...
    en=now(); receipts.append({
        "class":cls,"semantic":semantic,"started_utc":isoz(st),"ended_utc":isoz(en),
        "host":urllib.parse.urlparse(url).netloc,"path":urllib.parse.urlparse(url).path,
        "status":status,"bytes":len(raw),"elapsed_ms":int((time.monotonic()-t)*1000),
        "sha256":digest(raw) if raw else None,"error":err,
    })
    return status,raw
...
def collect_ioda(tag,code,receipts,t0):
    ...
    return {"status":"OK" if vals else "NO_SERIES","fresh":bool(latest and t0-latest<=timedelta(hours=6)),"latest_utc":isoz(latest),"points":len(vals),"trend":trend(vals)}
...
def main():
    t0=now(); receipts=[]; regions={}; af=rf=rel=both=0
