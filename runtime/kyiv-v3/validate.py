...
def main():
    ...
    receipt_rows=[json.loads(x) for x in R.read_text(encoding='utf-8').splitlines() if x.strip()];actual_digest=compact(receipt_rows);declared=p.get('receipt_digest');p['receipt_readback']={'lines':len(receipt_rows),'declared_digest':declared,'actual_digest':actual_digest,'digest_matches_declared':actual_digest==declared}
