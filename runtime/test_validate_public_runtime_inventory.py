...
class PublicRuntimeInventoryTests(unittest.TestCase):
    def _fixture(self, root: Path) -> tuple[Path, Path]:
        ...
        (runtime / "validate_public_runtime_inventory.py").write_text(
            "# validator\n", encoding="utf-8"
        )
        ...
        (config / "module_registry.json").write_text(
            json.dumps(
                {
                    "modules": [
                        {"module_id": "public-runtime-bridge", "state": "ENABLED"}
                    ],
    ...
    def test_complete_bound_inventory_passes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(result["secret_pull_request_trigger_absent"])
    ...
    def test_secret_bearing_pull_request_workflow_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            public, private = self._fixture(Path(tmp))
            workflow = public / ".github/workflows/runtime-self-test.yml"
            workflow.write_text(
                "name: Unsafe\non:\n  pull_request:\njobs:\n  x:\n    env:\n"
                "      TOKEN: ${{ secrets.COMMAND_CENTER_TOKEN }}\n",
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ValueError, "secret-bearing workflow"):
                validate_inventory(public_root=public, command_center=private)
