...
| History remediation repeats paid search | Recovery consumes another query credit and can duplicate current results | Schema v3 accepts only a prior private receipt reference; the worker privately derives at most two prior search IPs and allowlists only `/shodan/host/{ip}?history=true`; `/count`, `/api-info`, `/search`, and `/scan` are structurally unavailable and the declared query-credit budget is zero | Never |
| Large or malformed host history | Useful history is discarded with an opaque error or overflows the private receipt | History requests use provider minification, a bounded 16 MiB response ceiling and at most 25 normalized records per host; precise parser/size diagnostics are preserved and no automatic retry occurs | Never |
